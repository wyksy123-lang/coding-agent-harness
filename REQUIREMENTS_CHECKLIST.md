...
| R052 | 要求1 §4.8 | CI（GitHub Actions）必须配置：每次 push 自动运行测试 | .github/workflows/ | push 后 CI 自动触发 | DONE | T24: `.github/workflows/ci.yml` triggers push + PR to main; jobs `test`/`lint`/`typecheck` use Python 3.11, pip cache, `make test`/`make lint`/`make typecheck` |
